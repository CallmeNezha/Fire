    ...
    , pattern_group_(new QActionGroup(this))
{
    ...
    // FIND PREVIOUS
    layout->addWidget(&find_prev_);
    find_prev_.setText("Find Previous");
    connect(&find_prev_, SIGNAL(clicked()), this, SIGNAL(prevBtnClicked()));

    // FIND NEXT
    layout->addWidget(&find_next_);
    find_next_.setText("Find Next");
    connect(&find_next_, SIGNAL(clicked()), this, SIGNAL(nextBtnClicked()));

    // CLOSE BUTTON
    layout->addWidget(&close_);
    close_.setMaximumWidth(25);
    close_.setFocusPolicy(Qt::NoFocus);
    close_.setStyleSheet("* { border: none; }");
    close_.setIcon(style()->standardIcon(QStyle::SP_TitleBarCloseButton));
...
//    optionsButton->setIcon(style()->standardIcon(QStyle::SP_TitleBarCloseButton));
    ...
    QWidgetAction *optionsAction = new QWidgetAction(&search_);
    optionsAction->setDefaultWidget(optionsButton);
    search_.addAction(optionsAction, QLineEdit::LeadingPosition);
}
